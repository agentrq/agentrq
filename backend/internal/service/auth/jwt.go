...
type TokenService interface {
	...
	CreateOAuthStateToken(redirectURL, provider, workspaceID string) (string, error)
...
func (s *tokenService) CreateOAuthStateToken(redirectURL, provider, workspaceID string) (string, error) {
	now := time.Now()
	claims := StateClaims{
		RegisteredClaims: jwt.RegisteredClaims{
			Subject:   "oauth_state",
