...
// Returns the verified base62 workspace ID.
func (c *controller) HandleOAuthCallback(ctx context.Context, state string, code string, redirectURI string) (string, error) {
	...
	if !security.Verify(workspaceID62, c.tokenKey, signature) {
		zlog.Warn().Str("state", state).Msg("CSRF: Slack OAuth callback signature verification failed")
		return workspaceID62, fmt.Errorf("slack: invalid state signature")
	}

	workspaceID := monoflake.IDFromBase62(workspaceID62).Int64()
	if workspaceID == 0 {
		return workspaceID62, fmt.Errorf("slack: invalid workspace ID state: %s", workspaceID62)
	}

	ws, err := c.repo.SystemGetWorkspace(ctx, workspaceID)
	if err != nil {
		return workspaceID62, fmt.Errorf("slack: workspace not found: %w", err)
	}

	token, teamID, botUserID, authedUserID, err := c.slack.ExchangeCode(ctx, code, redirectURI)
	if err != nil {
		return workspaceID62, fmt.Errorf("slack: oauth exchange failed: %w", err)
	}

	encToken, nonce, err := security.Encrypt(token, c.tokenKey)
	if err != nil {
		return workspaceID62, fmt.Errorf("slack: failed to encrypt token: %w", err)
	}
	...
	if link.SlackChannelID == "" {
		...
		if err != nil {
			return workspaceID62, fmt.Errorf("slack: failed to auto-provision channel: %w", err)
		}
	...
	if err := c.repo.UpsertSlackWorkspaceLink(ctx, link); err != nil {
		return workspaceID62, fmt.Errorf("slack: failed to save workspace link: %w", err)
	}
