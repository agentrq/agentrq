...
describe('an extension that asks only to review tool calls', () => {
  ...
  it('still has a grant to record', () => {
    ...
    expect(grant.valid.value).toBe(true);
    // The workspaces it may be asked about *are* its grant, so it is asked
    // which — being consulted about a tool call is reading that workspace by
    // another name, and the main process refuses a reviewer whose grant does
    // not reach the workspace the request came from.
    expect(grant.scopes.value).toEqual([SCOPE.workspace, SCOPE.selected]);
    expect(grant.grant.value.workspaces).toEqual(['ws1']);
  });

  /**
   * The install screen lives in the sidebar and belongs to no workspace, so a
   * reviewer installed from there has to name the workspaces it may review in.
   * Falling back to "this workspace" there produced an empty list: a grant that
   * reaches nothing, an extension that is never consulted, and a row saying it
   * may refuse things.
   */
  it('names workspaces when installed from a screen that has none', () => {
    const grant = useExtensionGrant({ manifest: reviewer('deny'), workspaces: [{ id: 'ws1' }] });

    expect(grant.scopes.value).toEqual([SCOPE.selected]);
    expect(grant.valid.value).toBe(false);

    grant.selected.value = ['ws1'];
