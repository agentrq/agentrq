    ...
    <div class="w-full max-w-lg bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl shadow-sm max-h-[85vh] overflow-y-auto custom-scrollbar">
      ...
      <div class="px-5 py-4 flex flex-col gap-4">
        ...
        <section v-if="scopes.length > 0">
          <!-- The heading changes for a reviewer, because for one it is not a
               question about reach at all. An extension that calls nothing
               still has to be told which workspaces it may be *asked* about:
               seeing a tool call is seeing that workspace's arguments, and the
               main process refuses to consult a reviewer whose grant does not
               cover where the request came from. -->
          <h3 class="text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-zinc-500 mb-2">
            {{ ask.level === 'none' ? 'Where it may be asked' : 'What it may reach' }}
          </h3>
