...
## 2026-06-27 - OAuth CSRF via Signed State and Session-Bound Nonces
**Vulnerability:** The Slack OAuth flow used a raw workspace ID as the `state` parameter, and Google/GitHub flows used signed state without binding it to the user session. This allowed for CSRF attacks where an attacker could force a user to link a malicious Slack workspace or hijack an OAuth flow.
**Learning:** Signed `state` parameters prevent tampering but don't prevent replay or cross-session attacks unless they include a nonce that is also stored in a secure, session-bound cookie. If the validation logic or callback handlers don't strictly enforce these checks (e.g., by falling back to defaults or skipping checks if the cookie is missing), the protection is bypassed.
**Prevention:** Always use cryptographically signed `state` tokens (e.g., JWT) in all OAuth flows and include a random nonce that is verified against a secure, HTTP-only `oauth_state` cookie. Ensure validation is terminal (aborts the flow on failure) and that the nonce check is mandatory if present in the token. Consume and clear the cookie immediately upon validation.
