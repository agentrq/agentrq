...
A session created with no provider/model fails every turn outright (prompt assembly has no value for `{{model}}`), and one created with no `cwd` groups as "Ungrouped" instead of under your project in a capable UI (prompt assembly also has no value for `{{cwd}}`). So every dedicated session gets an explicit `cwd` (`cwd` config, or the dsh process's own cwd) and `agentOptions` (`provider`/`model` config, or — when those are unset — whichever live agent in the same process most recently started a turn, tracked from `agent/status` since the manager started; falling further back to the longest-lived agent still around when nothing has been active yet). There is no way to ask the harness for its own configured default model/cwd from a plugin, only to read them off an agent that already has them, so the very first task pushed before this process has ever seen any activity still fails unless `model` is configured explicitly.
...
**A newly opened session's first message leads with the task's title.** The framing itself always starts with a `[AGENTRQ]`/`[AGENTRQ TASK]` tag line, so a UI that names a session after its first message would otherwise show that tag rather than anything about the task. When the push that opens a session already carries the task (the startup catch-up path), its `title` is used directly; a plain workspace push carries no structured title, so the manager fetches one via `getTask` — one extra round trip, only the first time a task's session opens, and skipped entirely (silently, with a warning) if the lookup fails or the task has no title.
...
| `cwd` | `''` | Working directory for each task's dedicated session. Empty uses the dsh process's own cwd |
...
- **No surfaced way to find a task's session** — a task's dedicated session is an ordinary root agent, so it exists independently of any chat you have open, but there is no in-harness command yet to list or switch to one; you find it the same way you'd find any other root agent your dsh UI happens to expose (in `dsh web`, it lands in the sidebar grouped under `cwd`, same as any other session there).
