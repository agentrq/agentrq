...
It also adds a **Guardrail** tab to any workspace's settings, where its extra
rules are typed. That tab is the clearest thing in here about the split between
the two kinds of settings: the rules are a block of text with a shape the
extension invented, on a screen it describes and AgentRQ draws, stored per
workspace under `ctx.storage` — none of which the manifest knows anything
about.
