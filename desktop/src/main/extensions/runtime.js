 ...
 * @param {object} [deps.storageStore] From `storage.js`.
...
export function createRuntime({
  ...
  configStore,
  /**
   * Where extensions keep settings of their own shape.
   *
   * Defaulted to something inert rather than required, because only two things
   * here touch it — handing it to the host, and emptying it on uninstall — and
   * a runtime assembled for a test that does neither should not have to pass a
   * store to prove it.
   */
  storageStore = { forget: async () => ({ ok: true }) },
  readManifest,
  /**
  ...
  return {
    ...
    async remove(name) {
      ...
      // The whole reason the container is ours rather than the extension's: an
      // extension that wrote its settings to a file of its own would leave them
      // behind here, and nothing in the app would know they were there.
      await storageStore.forget(name)
